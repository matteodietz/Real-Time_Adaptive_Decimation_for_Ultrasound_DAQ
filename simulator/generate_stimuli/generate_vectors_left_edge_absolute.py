"""
Generate simulation vectors for find_bw_left_edge_absolute.sv module.
...
def clip_accumulator_to_32bit(accum_val_64bit, accum_frac_64=56):
    """
    Clip 64-bit accumulator (Q8.56) to 32-bit representation.
...
def generate_test_case(test_name, iq_data, fs, freq_bins, threshold_drop_db, 
    ...
    # --- 6. Find Left Edge (returns frequencies) ---
    f1_golden_freq, f2_golden_freq, L1_golden, L2_golden = find_left_edge_hw(
...
def write_vector_file(test_cases, output_path, accum_width, freq_bin_width):
    with open(output_path, 'w') as f:
        f.write("# Simulation vectors for find_bw_left_edge_absolute.sv\n")
...
def main():
    print("=== Generating Simulation Vectors for find_bw_left_edge_absolute.sv ===")
    ...
    if PICMUS_AVAILABLE:
        ...
        try:
            ...
            # Synthetic Test Case
            db_data = np.full(len(S_bins), 20)
            db_data[2:4] = 90
            db_data[8:11] = 90  # High power in middle-left
