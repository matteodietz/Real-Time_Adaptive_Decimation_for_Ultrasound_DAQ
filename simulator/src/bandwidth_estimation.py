...
if __name__ == '__main__':
    ...
    # --- NEW: Inject -60dB White Noise ---
    print("Injecting -60dB white noise...")
    # Calculate peak amplitude of the signal
    max_amp = np.max(np.abs(baseline_iq_data))
    
    # Calculate noise amplitude (-60dB relative to peak)
    target_snr_db = 60
    noise_scale = max_amp * (10 ** (-target_snr_db / 20))
    
    # Generate complex Gaussian noise
    # We divide by sqrt(2) so the magnitude of the complex vector matches noise_scale
    noise = (np.random.normal(0, 1, baseline_iq_data.shape) + 
             1j * np.random.normal(0, 1, baseline_iq_data.shape)) * (noise_scale / np.sqrt(2))
    
    # Add noise to data
    baseline_iq_data = baseline_iq_data + noise
    # -------------------------------------
    ...
    # --- NEW: Determine Global Reference Power ---
    # We use the max power of the *first* window in our list to normalize all others
    ref_time_idx = indices_to_plot[0]
    ref_spectrum_slice = avg_spectrogram[:, ref_time_idx]
    global_ref_max = np.max(ref_spectrum_slice)
    # ---------------------------------------------
    ...
    for i, time_idx in enumerate(indices_to_plot):
        ...
        # --- UPDATED: Normalize to the Global Reference ---
        # 10*log10( P_current / P_ref_max )
        # We add 1e-20 to numerator to avoid log(0)
        spectrum_db_normalized = 10 * np.log10((spectrum_slice + 1e-20) / global_ref_max)
        # --------------------------------------------------

        # find the -20dB bandwidth edges
        # Note: Since we are normalizing to the first window, deep windows might 
        # drop entirely below -20dB relative to the first window. 
        threshold = -20 # dB
        ...
        if len(above_threshold_indices) > 0:
            ...
            # Avoid division by zero if freq is 0
            if max_abs_freq > 0:
                practical_margin = 2.5 # oversampling
                fs_min = practical_margin * max_abs_freq
                M_max = adc_rate / fs_min
    ...
    # configure plot labels and style
    plt.title('Normalized Power Spectrum at Different Imaging Depths (Ref: Max of Shallowest Window)')
    plt.xlabel('Frequency Offset (MHz)')
    plt.ylabel('Power (dB relative to max of first window)')
    plt.grid(True, which='both', linestyle='--')
    plt.legend()
    
    # Adjust Y-limit to show the noise floor (approx -60dB)
    plt.ylim(-70, 5) 
    
    # xlim uses the actual baseline sample rate
    plt.xlim(-fs_baseline/4 / 1e6, fs_baseline/4 / 1e6)
